...
bool Audio::open(AVStream *stream, const std::shared_ptr<ClockSync> &clock, const std::function<void()> &onEof)
{
	...
	// if (_forceFormat == AV_SAMPLE_FMT_NONE)
	{
		...
		if (_forceFormat == AV_SAMPLE_FMT_NONE)
		{
			...
			{
			case AUDIO_S16SYS:
				_forceFormat = AV_SAMPLE_FMT_S16;
				break;
			case AUDIO_S32SYS:
				_forceFormat = AV_SAMPLE_FMT_S32;
				break;
			case AUDIO_F32SYS:
				_forceFormat = AV_SAMPLE_FMT_FLT;
				break;
			default:
...
void Audio::_displayThread()
{
	...
	while (_isAlive)
	{
		...
		if (_forceFormat != frame->format || _spec.freq != frame->sample_rate || _spec.channels != frame->ch_layout.nb_channels)
		{
			...
			if ((out->nb_samples = swr_convert(_swrContext, out->data, out->nb_samples, (const uint8_t **)frame->extended_data, frame->nb_samples)) < 0)
				goto end_loop;
		}
		else
		{
		...
		while (avail)
		{
			if (!writtenSamples)
			{
				while (!(buf = _channel->frameQueue.begin_write(_frameAlignBytes)))
					co_sleep(10);
			}
